...
from .maskable_lstm_buffers_samples import (
...
class MaskableRecurrentRolloutBuffer(RolloutBuffer):
    ...
    def add(self, *args, action_masks: Optional[np.ndarray] = None, lstm_states: RNNStates, **kwargs,) -> None:
        """
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[MaskableRecurrentRolloutBufferSamples, None, None]:
        ...
        # TODO: Remove this if some error appears
    ...
    def _get_samples(
        ...
        return MaskableRecurrentRolloutBufferSamples(
            ...
            action_masks=pad_and_flatten(self.action_masks[batch_inds].reshape(-1, self.mask_dims))
        )
...
class MaskableRecurrentDictRolloutBuffer(DictRolloutBuffer):
    ...
    def _get_samples(
        ...
        return MaskableRecurrentDictRolloutBufferSamples(
            ...
            episode_starts=self.pad_and_flatten(self.episode_starts[batch_inds]),
            mask=self.pad_and_flatten(np.ones_like(self.returns[batch_inds])),
            action_masks=self.pad_and_flatten(self.action_masks[batch_inds].reshape(-1, self.mask_dims))
        )
